        ...
        if retrain_decision['should_retrain']:
            ...
            if retrain_decision['priority'] in ['critical', 'high']:
                ...
                retrain_task = await trigger_vectorizer_retrain.kiq({
                    ...
                    'details': retrain_decision['details'],
                    'force': False
                }, _deferred_by=countdown_seconds)  # Note: TaskIQ doesn't have native delay, we use a custom workaround? 
                # Actually, we cannot delay a TaskIQ task natively in the same way as Celery.
                # We will ignore the delay for now and note that this is a limitation.
                # In a production system, we would use a different mechanism (like a scheduler) for delayed tasks.
                # For the purpose of this migration, we will just run the retrain task immediately and note the limitation.
...
async def check_model_health_task() -> Dict[str, Any]:
    ...
    try:
        # Import des services nécessaires
        from backend_worker.services.model_persistence_service import ModelPersistenceService, ModelVersioningService
        from datetime import datetime
