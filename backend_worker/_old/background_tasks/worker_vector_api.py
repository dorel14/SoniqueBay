"""
API HTTP pour le worker vector - Endpoints de communication avec recommender_api
Permet la communication HTTP entre library_api et backend_worker sans imports directs.
"""
...
async def publish_vectorization_event(data: Dict[str, Any]):
    ...
    try:
        ...
        # Publier via Redis (méthode alternative à l'import direct)
...
async def trigger_batch_vectorization(track_ids: List[int]):
    ...
    try:
        ...
        from backend_worker.background_tasks.worker_vector import vectorize_tracks_batch_task
        result = vectorize_tracks_batch_task.delay(track_ids, "normal")
...
async def trigger_single_vectorization(track_id: int):
    ...
    try:
        ...
        # Lancer la tâche Celery
        from backend_worker.background_tasks.worker_vector import vectorize_single_track_task
        result = vectorize_single_track_task.delay(track_id)
...
async def get_vectorization_status():
    ...
    try:
        ...
        # Note: Cette méthode devrait être asynchrone dans le service
