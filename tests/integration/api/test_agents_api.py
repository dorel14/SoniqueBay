...
def test_create_agent(client: TestClient):
    ...
    data = response.json()
    assert data["name"] == "test_agent_create"
...
def test_list_agents(client: TestClient):
    ...
    # Doit contenir au moins les agents créés précédemment
    assert len(data) >= 2
...
def test_get_agent(client: TestClient):
    ...
    # Récupérer l'agent
    response = client.get("/api/agents/test_agent_get")
    assert response.status_code == 200
    data = response.json()
    assert data["name"] == "test_agent_get"
    assert data["model"] == "gpt-4"


def test_get_agent_not_found(client: TestClient):
    """Tester la récupération d'un agent inexistant."""
    response = client.get("/api/agents/nonexistent_agent")
    assert response.status_code == 404


def test_update_agent(client: TestClient):
    """Tester la mise à jour d'un agent."""
    # Créer un agent d'abord
    payload = {
...
def test_delete_agent(client: TestClient):
    """Tester la suppression d'un agent."""
    # Créer un agent d'abord
    payload = {
    ...
    # Supprimer l'agent
    response = client.delete("/api/agents/test_agent_delete")
    assert response.status_code == 200
    assert response.json()["status"] == "deleted"
    
    # Vérifier qu'il n'existe plus
    response = client.get("/api/agents/test_agent_delete")
    assert response.status_code == 404


def test_delete_agent_not_found(client: TestClient):
    """Tester la suppression d'un agent inexistant."""
    response = client.delete("/api/agents/nonexistent_agent")
    assert response.status_code == 404


def test_list_tools(client: TestClient):
    """Tester la liste des outils disponibles."""
    response = client.get("/api/agents/ai/tools")
    assert response.status_code == 200
    data = response.json()
    assert isinstance(data, list)
...
def test_create_agent_score(client: TestClient):
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["agent_name"] == "test_agent_score"
...
def test_get_agent_score(client: TestClient):
    ...
    # Créer un score d'abord
    payload = {
        "agent_name": "test_agent_get_score",
    ...
    # Récupérer le score
    response = client.get("/api/agents/scores/test_agent_get_score/test_intent_get")
    assert response.status_code == 200
    data = response.json()
    assert data["agent_name"] == "test_agent_get_score"
    assert data["intent"] == "test_intent_get"


def test_get_agent_score_not_found(client: TestClient):
    """Tester la récupération d'un score d'agent inexistant."""
    response = client.get("/api/agents/scores/nonexistent/nonexistent")
    assert response.status_code == 404
...
def test_list_agent_scores_with_filters(client: TestClient):
    """Tester la liste des scores avec filtres."""
...
def test_list_agent_scores_with_pagination(client: TestClient):
    """Tester la liste des scores avec pagination."""
...
def test_update_agent_score(client: TestClient):
    ...
    # Créer un score
    payload = {
        "agent_name": "test_agent_update_score",
    ...
    response = client.put(
        "/api/agents/scores/test_agent_update_score/test_intent_update",
        json=update_payload
    )
    assert response.status_code == 200
    data = response.json()
...
def test_update_agent_score_not_found(client: TestClient):
    """Tester la mise à jour d'un score d'agent inexistant."""
    update_payload = {
        "score": 9.0,
        "usage_count": 10,
    }
    response = client.put(
        "/api/agents/scores/nonexistent/nonexistent",
        json=update_payload
    )
    assert response.status_code == 404
...
def test_increment_agent_score_usage_not_found(client: TestClient):
    """Tester l'incrémentation d'un score inexistant."""
    response = client.patch(
        "/api/agents/scores/nonexistent/nonexistent/usage?success=true"
    )
    assert response.status_code == 404
...
def test_delete_agent_score(client: TestClient):
    ...
    # Créer un score
    payload = {
        "agent_name": "test_agent_delete_score",
    ...
    # Supprimer le score
    response = client.delete("/api/agents/scores/test_agent_delete_score/test_intent_delete")
    assert response.status_code == 200
    assert response.json()["status"] == "deleted"
    
    # Vérifier qu'il n'existe plus
    response = client.get("/api/agents/scores/test_agent_delete_score/test_intent_delete")
    assert response.status_code == 404


def test_delete_agent_score_not_found(client: TestClient):
    """Tester la suppression d'un score d'agent inexistant."""
    response = client.delete("/api/agents/scores/nonexistent/nonexistent")
    assert response.status_code == 404
...
def test_get_agent_scores_with_metrics_filters(client: TestClient):
    """Tester la récupération des scores avec métriques et filtres."""
    # Créer des scores
    test_data = [
        {"agent_name": "metric_filter_1", "intent": "intent_a", "score": 8.0, "usage_count": 10, "success_count": 8},
        {"agent_name": "metric_filter_1", "intent": "intent_b", "score": 6.0, "usage_count": 5, "success_count": 2},
        {"agent_name": "metric_filter_2", "intent": "intent_a", "score": 9.0, "usage_count": 20, "success_count": 18},
    ]
    ...
    # Filtrer par agent_name
    response = client.get("/api/agents/scores/metrics?agent_name=metric_filter_1")
    assert response.status_code == 200
    data = response.json()
    assert data["count"] == 2
    assert all(s["agent_name"] == "metric_filter_1" for s in data["results"])
    
    # Filtrer par intent
    response = client.get("/api/agents/scores/metrics?intent=intent_a")
    assert response.status_code == 200
    data = response.json()
    assert data["count"] == 2
    assert all(s["intent"] == "intent_a" for s in data["results"])
