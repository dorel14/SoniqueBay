...
# Task dispatcher function - called by tasks
async def start_scan(directory: str, callback=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        await _maybe_trigger_gmm_clustering(result)
...
async def _maybe_trigger_gmm_clustering(scan_result: Dict[str, Any]) -> None:
    """
...
async def _trigger_gmm_clustering() -> None:
    """
    Déclenche la tâche TaskIQ de clustering GMM en arrière-plan.
    """
    try:
        from backend.tasks.gmm import cluster_all_artists_task

        logger.info("[SCAN] Déclenchement du clustering GMM post-scan")
        await cluster_all_artists_task.kiq(False)  # force_refresh=False
