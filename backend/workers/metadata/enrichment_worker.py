...
def extract_single_file_metadata(file_path: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        from backend.workers.services.music_scan import (
        ...
        def _clean_single_genre(single_genre):
            ...
            try:
                from backend.workers.utils.genre_converter import load_genre_library, normalize_genre, create_genre_mapping
                ...
                if normalized and normalized in valid_genres:
                    ...
                    try:
                        ...
                        from backend.workers.services.cache_service import cache_service
        ...
        try:
            ...
            try:
                ...
                if 'APIC:' in audio:
                    ...
                    try:
                        from backend.workers.services.image_service import convert_to_base64_sync
            ...
            # Extraction optimisée des tags audio (sans diagnostic redondant)
            try:
                all_tags = dict(audio) if hasattr(audio, 'keys') else {}
                
                # Convertir les tags en format sérialisable pour l'enrichissement différé
                serialized_tags = {}
                ...
                # Stocker uniquement si des tags audio pertinents sont présents
                ...
                if audio_tag_keys:
                    ...
                    logger.debug(f"[METADATA] {len(audio_tag_keys)} tags audio trouvés, stockés pour enrichissement différé")
                
            except Exception as e:
                logger.debug(f"[METADATA] Erreur lors de l'extraction des tags: {e}")
