...
@broker.task(name="batch.process_entities", queue="batch")
async def batch_entities(metadata_list: List[Dict[str, Any]], batch_id: str | None = None):
    ...
    try:
        ...
        for track in metadata_list:
            ...
            if track_tags:
                from backend.workers.services.audio_features_service import _has_valid_audio_tags
        ...
        if tracks_with_audio_tags:
            logger.info(f"[BATCH] {len(tracks_with_audio_tags)} tracks avec tags audio détectés, dispatch des tâches TaskIQ...")
            audio_tasks_sent = 0
            ...
            for track_info in tracks_with_audio_tags:
                try:
                    ...
                    if file_path:
                        # Dispatcher la tâche TaskIQ d'analyse audio
                        from backend.workers.tasks.audio_analysis_tasks import analyze_track_audio_features
                        analyze_track_audio_features.kiq(
                            track_id=None,
                            file_path=file_path,
                            tags=tags
                        )
        ...
        try:
            from backend.workers.insert.insert_batch_worker import insert_batch_direct
            insert_batch_direct.kiq(insertion_data=insertion_data)
            logger.info(f"[BATCH] Tâche insert.direct_batch envoyée")
        except Exception as e:
