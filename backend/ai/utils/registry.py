...
class ToolRegistry:
    ...
    def register(
        ...
        if name in cls._tools:
            logger.warning(
                f"Tool '{name}' déjà enregistré, mise à jour des métadonnées"
            )
    ...
    @classmethod
    def values(cls) -> List[AIToolMetadata]:
        """Retourne la liste des métadonnées des tools (compatibilité)."""
    ...
    def get_by_category(cls, category: str) -> Dict[str, AIToolMetadata]:
        ...
        return {
            name: tool for name, tool in cls._tools.items() if tool.category == category
        }
        ...
        **kwargs,
        ...
        if agent_name and not cls.validate_access(tool_name, agent_name):
            ...
            raise PermissionError(
                f"Agent '{agent_name}' non autorisé à utiliser le tool '{tool_name}'"
            )
        ...
        try:
            ...
            if tool.timeout:
                if inspect.iscoroutinefunction(func):
                    result = asyncio.wait_for(
                        func(*args, **kwargs), timeout=tool.timeout
                    )
                else:
            ...
            raise TimeoutError(
                f"Le tool '{tool_name}' a dépassé le timeout de {tool.timeout}s"
            )
    ...
    def get_statistics(cls) -> Dict[str, Any]:
        ...
        most_used = sorted(
            cls._tools.items(), key=lambda x: x[1].call_count, reverse=True
        )[:10]

        # Tools avec le plus d'erreurs
        most_errors = sorted(
            cls._tools.items(), key=lambda x: x[1].error_count, reverse=True
        )[:5]

        # Performance moyenne par catégorie
        avg_performance = {
            category: data["avg_time"] for category, data in categories.items()
        }
    ...
    def get_health_report(cls) -> Dict[str, Any]:
        ...
        for category, data in stats["categories"].items():
            if data["calls"] == 0:
                issues.append({"type": "unused_category", "category": category})
        ...
        return {
            ...
            "status": (
                "healthy"
                if len(issues) == 0
                else "warning" if len(issues) < 5 else "critical"
            ),
        }
    ...
    def _generate_recommendations(cls, issues: List[Dict], stats: Dict) -> List[str]:
        ...
        if error_issues:
            recommendations.append(
                f"Vérifier et corriger les {len(error_issues)} tools avec un taux d'erreur élevé"
            )

        slow_issues = [issue for issue in issues if issue["type"] == "slow_execution"]
        if slow_issues:
            recommendations.append(
                f"Optimiser les {len(slow_issues)} tools lents (> 5s d'exécution moyenne)"
            )

        if stats["success_rate"] < 0.95:
            recommendations.append(
                "Améliorer la fiabilité globale du système (taux de succès < 95%)"
            )

        if stats["total_calls"] == 0:
            recommendations.append(
                "Aucun tool n'a été exécuté récemment - vérifier l'intégration"
            )
