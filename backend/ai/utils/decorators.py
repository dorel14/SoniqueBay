        ...
        if allowed_agents is not None:
            ...
            for agent in allowed_agents:
                if not isinstance(agent, str) or not agent.strip():
                    raise ValueError(
                        "Les noms d'agents doivent être des chaînes non vides"
                    )
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            ...
            try:
                ...
                result = ToolRegistry.execute_with_monitoring(
                    tool_name=name, func=func, agent_name=agent_name, *args, **kwargs
                )
                ...
                if track_usage:
                    _track_tool_usage(
                        name, agent_name, start_time, success=False, error=str(e)
                    )
...
def _validate_session_parameter(func: Callable, args: tuple, kwargs: dict) -> None:
    ...
    raise ValueError(
        f"Le tool '{func._ai_tool_metadata['name']}' nécessite une session de base de données"
    )
