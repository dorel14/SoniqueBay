...
async def build_agent(agent_model: AgentModel) -> Agent:
    ...
    if not agent_model.role or not agent_model.task:
        raise ValueError(
            "Les champs ROLE et TASK sont requis pour un agent RTCROS valide"
        )
    ...
    try:
        ollama_model = await get_ollama_model(
            model_name=agent_model.model, num_ctx=agent_model.num_ctx
        )
    ...
    try:
        ...
        logger.error(
            ...
            extra={"agent_name": agent_model.name, "error": str(e)},
            exc_info=True,
        )
        raise ValueError(f"Impossible de créer l'agent: {e}")


async def build_agent_with_inheritance(
    agent_model: AgentModel, base_agents: Dict[str, AgentModel]
) -> Agent:
    """
    ...
    logger.info(
        ...
        extra={
            ...
            "base_agent": agent_model.base_agent,
            "specialized_tools": (
                len(child_tools) - len(parent_agent.tools)
                if hasattr(parent_agent, "tools")
                else 0
            ),
        },
    )
...
async def validate_agent_configuration(agent_model: AgentModel) -> Dict[str, Any]:
    ...
    for field in required_fields:
        if not getattr(agent_model, field):
            ...
            validation_report["issues"].append(
                f"Champ RTCROS obligatoire manquant: {field}"
            )
    ...
    if agent_model.base_agent:
        ...
        validation_report["details"][
            "inheritance"
        ] = f"Hérite de: {agent_model.base_agent}"
