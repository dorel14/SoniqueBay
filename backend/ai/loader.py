...
class AgentLoader:
    ...
    async def _load_base_agents(self) -> Dict[str, AgentModel]:
        ...
        result = await self.session.execute(
            select(AgentModel).where(
                AgentModel.enabled == True, AgentModel.base_agent.is_(None)
            )
        )
    ...
    async def _load_and_build_agents(
        self, base_agents: Dict[str, AgentModel]
    ) -> Dict[str, Any]:
        """
        ...
        result = await self.session.execute(
            select(AgentModel).where(
                AgentModel.enabled == True, AgentModel.base_agent.isnot(None)
            )
        )
        ...
        for name, model in base_agents.items():
            try:
                ...
                logger.error(
                    ...
                    extra={"agent_name": name, "error": str(e)},
                    exc_info=True,
                )
    ...
    async def load_agent_by_name(self, agent_name: str) -> Optional[Any]:
        ...
        result = await self.session.execute(
            select(AgentModel).where(
                AgentModel.name == agent_name, AgentModel.enabled == True
            )
        )
