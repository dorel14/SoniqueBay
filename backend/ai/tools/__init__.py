...
async def create_playlist(
    session, name: str, track_ids: List[int], description: str = ""
) -> Dict[str, Any]:
    """Crée une playlist."""
    return {
        "id": 1,
...
async def play_track(session, track_id: int) -> Dict[str, Any]:
    ...
    return {
        "status": "success",
        "message": f"Lecture démarrée pour le morceau {track_id}",
        "track": {"id": track_id},
    }
...
async def add_to_playqueue(session, track_id: int) -> Dict[str, Any]:
    ...
    return {
        "status": "success",
        "message": f"Ajouté à la file de lecture: morceau {track_id}",
    }
...
ToolRegistry.register(
    "add_to_playqueue", add_to_playqueue, "Ajout à la file de lecture"
)
...
ToolRegistry.register(
    "get_recommendations", get_recommendations, "Recommandations musicales"
)
