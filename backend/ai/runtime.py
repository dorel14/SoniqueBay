...
class StreamingBuffer:
    ...
    def should_flush(self) -> bool:
        ...
        return (
            len(self.chunks) >= 10  # Buffer assez gros
            or current_time - self.last_flush >= self.flush_interval  # Temps écoulé
        )
...
class AgentRuntime:
        ...
        for attempt in range(max_retries):
            try:
                result = await self._call_agent_with_retry(
                    self.agent.run, message, context, attempt
                )
    ...
    async def _stream_with_buffering(
        ...
        async for event in self._call_agent_stream_with_timeout(
            message, context, timeout=180
        ):
    ...
    # -------------------------------------------------
    async def _call_agent_with_retry(self, fn, message: str, context, attempt: int):
        ...
        try:
            return await asyncio.wait_for(
                self._call_agent(fn, message, context), timeout=timeout
            )
    ...
    async def _call_agent_stream_with_timeout(
        self, message: str, context, timeout: float = 180.0
    ):
        ...
        try:
            ...
            stream_gen = self._call_agent_stream(
                self.agent.run_stream, message, context
            )
            ...
            logger.warning(
                ...
                extra={"agent_name": self.name, "timeout": timeout},
            )
            raise
        except Exception as e:
            logger.error(
