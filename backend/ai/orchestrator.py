...
class Orchestrator:
    ...
    async def handle(self, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self._execute_agent_with_monitoring(
                agent_name=agent_name, message=message, intent=intent
            )
    ...
    # ---------------------------------------------------------
    async def _detect_intent_with_timeout(
        self, message: str, timeout: float = 10.0
    ) -> Dict[str, Any]:
        """Détection d'intention avec timeout et fallback."""
        try:
    ...
    async def _select_agent_with_fallback(
        ...
        logger.warning(
            f"Aucun agent trouvé pour l'intention '{intent}', fallback vers smalltalk_agent"
        )
    ...
    async def _execute_agent_with_monitoring(
        self, agent_name: str, message: str, intent: Optional[str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                ...
                extra={"agent_name": agent_name, "intent": intent, "error": str(e)},
                exc_info=True,
            )
            raise
    ...
    async def _handle_orchestration_error(
        self, error: Exception, message: str, start_time: float
    ) -> Dict[str, Any]:
    ...
    async def _get_fallback_agent(
        ...
        return (
            "smalltalk_agent"
            if "smalltalk_agent" in self.agents
            else list(self.agents.keys())[0]
        )

    def _update_stats(
        self, success: bool, response_time: float, agent_name: str
    ) -> None:
    ...
    def get_health_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_healthy": self._stats["failed_requests"]
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        success_rate = self._stats["successful_requests"] / max(
            1, self._stats["total_requests"]
        )
        ...
        return {
            ...
            "total_requests": self._stats["total_requests"],
            "error_rate": self._stats["failed_requests"]
            / max(1, self._stats["total_requests"]),
