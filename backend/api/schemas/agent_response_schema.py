...
class AgentToolCall(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Paramètres de l'outil"
    )
    tool_call_id: Optional[str] = Field(
        None, description="ID unique pour l'appel d'outil"
    )
...
class AgentClarificationRequest(BaseModel):
    ...
    clarification_id: Optional[str] = Field(
        None, description="ID unique pour la demande de clarification"
    )
...
class AgentToolResult(BaseModel):
    ...
    result: Optional[Dict[str, Any]] = Field(
        None, description="Données renvoyées par le tool"
    )
...
class AgentMessageResponse(BaseModel):
    ...
    tool_call: Optional[AgentToolCall] = Field(
        None, description="Appel d'outil si type=tool_call"
    )
    tool_result: Optional[AgentToolResult] = Field(
        None, description="Résultat d'un tool exécuté"
    )
    clarification: Optional[AgentClarificationRequest] = Field(
        None, description="Demande de clarification si type=clarification"
    )
    ...
    agent_avatar: Optional[str] = Field(
        None, description="Identifiant/avatar de l'agent pour l'UI"
    )
...
class SearchAgentResponse(BaseModel):
    ...
    results: List[Track] = Field(
        default_factory=list, description="Résultats de recherche"
    )
...
class SmalltalkAgentResponse(ChatResponse):
    ...
    mood: Optional[str] = Field(
        None, description="Humeur détectée dans la conversation"
    )
    sentiment_score: Optional[float] = Field(
        None, description="Score de sentiment (-1 à 1)"
    )
