...
class AlbumBase(BaseModel):
    ...
    musicbrainz_albumid: Optional[str] = Field(
        None, description="ID MusicBrainz de l'album"
    )

    # Champ optionnel pour résoudre album_artist_name -> album_artist_id
    album_artist_name: Optional[str] = Field(
        None,
        description="Nom de l'artiste (pour résolution automatique)",
        deprecated=True,
    )
    ...
    def validate_release_year(cls, v):
        ...
        try:
            ...
            logger.warning(
                f"Erreur conversion release_year: {v} ({type(v).__name__}) - {e}"
            )
            return None
    ...
    def validate_artist_reference(cls, data):
        ...
        if isinstance(data, dict):
            ...
            has_artist_id = (
                "album_artist_id" in data and data["album_artist_id"] is not None
            )
            ...
            if has_artist_id:
                ...
                if has_artist_name:
                    logger.debug(
                        f"album_artist_id fourni ({data['album_artist_id']}), album_artist_name ignoré"
                    )
                return data
...
class AlbumUpdate(BaseModel):
    title: Optional[str] = Field(None, description="Titre de l'album")
    album_artist_id: Optional[int] = Field(None, description="ID de l'artiste")
    release_year: Optional[str] = Field(None, description="Année de sortie")
    musicbrainz_albumid: Optional[str] = Field(
        None, description="ID MusicBrainz de l'album"
    )
    ...
    @classmethod
    def validate_release_year(cls, v):
        return AlbumBase.validate_release_year(v)
