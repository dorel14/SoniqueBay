...
class ArtistEmbeddingCreate(BaseModel):
    ...
    cluster_probabilities: Optional[Dict[int, float]] = Field(
        None, description="Probabilities for each cluster"
    )
...
class ArtistEmbeddingUpdate(BaseModel):
    ...
    vector: Optional[List[float]] = Field(
        None, description="Updated vector representation"
    )
    cluster: Optional[int] = Field(None, description="Updated cluster assignment")
    cluster_probabilities: Optional[Dict[int, float]] = Field(
        None, description="Updated cluster probabilities"
    )
...
class GMMTrainingRequest(BaseModel):
    ...
    n_components: int = Field(
        ..., ge=1, le=50, description="Number of Gaussian components"
    )
    max_iterations: int = Field(
        100, ge=1, le=1000, description="Maximum number of EM iterations"
    )
    convergence_threshold: float = Field(
        1e-3, gt=0, description="Convergence threshold for EM algorithm"
    )
...
class ArtistSimilarityRecommendation(BaseModel):
    ...
    similar_artists: List[Dict[str, Any]] = Field(
        default_factory=list, description="List of similar artists"
    )
    cluster_based: bool = Field(
        True, description="Whether recommendations are cluster-based"
    )
    similarity_score: Optional[float] = Field(
        None, description="Overall similarity score"
    )
    ...
    source: Optional[str] = Field(
        None, description="Source of recommendation (e.g., 'vector_similarity')"
    )
