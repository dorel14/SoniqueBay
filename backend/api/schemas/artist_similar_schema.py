...
class ArtistSimilarBase(BaseModel):
    ...
    weight: float = Field(
        ..., description="Similarity weight (0.0 to 1.0)", ge=0.0, le=1.0
    )
...
class ArtistSimilarUpdate(BaseModel):
    ...
    weight: Optional[float] = Field(
        None, description="Updated similarity weight", ge=0.0, le=1.0
    )
...
class ArtistSimilar(ArtistSimilarBase, TimestampedSchema):
    ...
    id: int = Field(
        ..., description="Unique identifier for the similarity relationship"
    )
...
class ArtistSimilarWithDetails(ArtistSimilar):
    ...
    similar_artist_name: Optional[str] = Field(
        None, description="Name of the similar artist"
    )
...
class ArtistSimilarListResponse(BaseModel):
    ...
    results: List[ArtistSimilarWithDetails] = Field(
        ..., description="List of artist similarity relationships"
    )
