...
from backend.api.schemas import EmbeddingRequest, EmbeddingResponse
...
async def publish_vectorization_event(data: Dict[str, Any]):
    ...
    try:
        ...
        # Publier via Redis
        from backend.utils.redis_utils import publish_vectorization_event as publish_event
...
async def trigger_batch_vectorization(track_ids: List[int]):
    ...
    try:
        ...
        # Lancer la tâche TaskIQ via le nouveau système
        from backend.tasks.vectorization import vectorize_tracks_batch
        result = await vectorize_tracks_batch.kiq(track_ids)
...
async def trigger_single_vectorization(track_id: int):
    ...
    try:
        ...
        # Lancer la tâche TaskIQ via le nouveau système
        from backend.tasks.vectorization import vectorize_single_track
        result = await vectorize_single_track.kiq(track_id)
...
async def trigger_vectorizer_training():
    ...
    try:
        ...
        # Lancer la tâche TaskIQ d'entraînement via le nouveau système
        from backend.tasks.vectorization import train_vectorizer
        result = await train_vectorizer.kiq()
...
async def get_vectorization_status():
    ...
    try:
        ...
        from backend.services.vectorization_service import VectorizationService
...
async def generate_embedding(request: EmbeddingRequest):
    ...
    """
    try:
        from backend.services.ollama_embedding_service import (
