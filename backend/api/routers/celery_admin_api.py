...
async def get_failed_tasks(limit: int = 100):
    ...
    try:
        ...
        logger.error(f"[CELERY ADMIN] Erreur lors de la récupération des tâches en échec: {e}")
...
async def retry_failed_task(request: RetryTaskRequest):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[CELERY ADMIN] Erreur parsing args pour tâche {request.task_id}: {e}")
            return RetryTaskResponse(
                success=False,
        ...
        logger.info(f"[CELERY ADMIN] Tâche {request.task_id} ({task_name}) relancée avec succès, nouveau ID: {result.id}")
        ...
        logger.error(f"[CELERY ADMIN] Erreur lors du retry de la tâche {request.task_id}: {e}")
...
async def delete_failed_task(task_id: str):
    ...
    try:
        ...
        if not client.exists(key):
            raise HTTPException(
                status_code=404,
                detail=f"Tâche {task_id} non trouvée dans la DLQ"
            )
        ...
        logger.error(f"[CELERY ADMIN] Erreur lors de la suppression de la tâche {task_id}: {e}")
