...
async def list_genre_tags(db: AsyncSession = Depends(get_async_session)):
    service = TagService(db)
    return await service.get_genre_tags()


@router.get("/mood-tags/", response_model=List[Tag])
async def list_mood_tags(db: AsyncSession = Depends(get_async_session)):
    service = TagService(db)
    return await service.get_mood_tags()
...
async def get_tags_by_type(type: str, db: AsyncSession = Depends(get_async_session)):
    service = TagService(db)
    if type == "mood":
        return await service.get_mood_tags()
    elif type == "genre":
