...
@router.get(
    "/path-variables", description="Récupère les variables disponibles pour les chemins"
)
...
async def create_setting(
    setting: SettingCreate, db: AsyncSession = Depends(get_async_session)
):
    try:
        from backend.api.services.settings_service import SettingsService
...
async def update_setting(
    key: str, setting: SettingCreate, db: AsyncSession = Depends(get_async_session)
):
    from backend.api.services.settings_service import SettingsService
