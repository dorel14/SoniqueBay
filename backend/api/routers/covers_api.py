...
PLACEHOLDER_PATH = (
    Path(__file__).resolve().parent.parent.parent / "data" / "static" / "logo.png"
)


@router.post("/", response_model=CoverSchema)
async def create_cover(
    cover: CoverCreate, db: AsyncSession = Depends(get_async_session)
):
...
async def serve_cover(
    entity_type: str, entity_id: int, db: AsyncSession = Depends(get_async_session)
):
    """
    Sert une cover WebP.
    ...
    logger.info(
        f"[COVER API] Appel de get_cover_path avec entity_type={entity_type}, entity_id={entity_id}"
    )
    ...
    if cover_db and getattr(cover_db, "cover_data", None):
        try:
            logger.info(
                f"[COVER API] Conversion base64 → WebP pour {entity_type}/{entity_id}"
            )
            ...
            raise HTTPException(
                status_code=500, detail=f"Erreur conversion WebP: {str(e)}"
            )

    # Fallback placeholder
    logger.warning(
        f"[COVER API] Aucune cover trouvée pour {entity_type}/{entity_id}, utilisation du placeholder"
    )
...
async def update_cover(
    ...
    if db_cover is None:
        raise HTTPException(
            status_code=400, detail=f"Type d'entité invalide: {entity_type}"
        )
...
async def scan_artist_images(
    ...
    try:
        ...
        logger.error(
            f"[COVER API] Erreur lors du scan des images de l'artiste {artist_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/scan/album/{album_id}")
async def scan_album_images(
    ...
    try:
        ...
        logger.error(
            f"[COVER API] Erreur lors du scan des images de l'album {album_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def scan_all_artist_images(db: AsyncSession = Depends(get_async_session)):
    ...
    try:
        ...
        logger.error(
            f"[COVER API] Erreur lors du scan des images des artistes: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def scan_embedded_covers(
    directory: Optional[str] = None, db: AsyncSession = Depends(get_async_session)
):
    """Relance le scan des covers intégrées dans les fichiers musicaux."""
    try:
        logger.info(
            f"[COVER API] Début scan des covers intégrées{' dans ' + directory if directory else ''}"
        )
...
async def rescan_artist_images(
    ...
    try:
        ...
        logger.error(
            f"Erreur lors du rescan des images de l'artiste {artist_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def rescan_album_covers(
    ...
    try:
        ...
        logger.error(
            f"Erreur lors du rescan des covers de l'album {album_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def rescan_track_embedded_cover(
    track_id: int, db: AsyncSession = Depends(get_async_session)
):
    ...
    try:
        ...
        logger.error(
            f"Erreur lors du rescan de la cover de la track {track_id}: {str(e)}"
        )
        raise HTTPException(status_code=500, detail=str(e))
