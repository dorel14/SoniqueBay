...
class ResilientRedisBackend(RedisBackend):
    ...
    async def get(self, key: str) -> Optional[bytes]:
        ...
        try:
            ...
            logger.warning(
                f"[CACHE] Returning None for key '{key}' due to Redis loading state"
            )
            return None
        except Exception as e:
    ...
    async def delete(self, key: str) -> None:
        ...
        try:
            ...
            logger.warning(
                f"[CACHE] Skipping cache delete for key '{key}' due to Redis loading state"
            )
        except Exception as e:
    ...
    async def clear(
        ...
        try:
            return await self._execute_with_retry(
                "clear", super().clear, namespace, key
            )
        except redis.exceptions.BusyLoadingError:
...
def create_resilient_redis_backend(
    ...
    return ResilientRedisBackend(
        redis=redis_client, max_retries=max_retries, retry_delay=retry_delay
    )
