...
def deserialize_queues_from_redis(queues_data: Dict[str, str]) -> List[Queue]:
    ...
    for queue_name, queue_json in queues_data.items():
        try:
            ...
            logger.warning(
                f"[CELERY_CONFIG_LOADER] Erreur désérialisation queue {queue_name}: {e}"
            )
            continue
...
def deserialize_routes_from_redis(routes_data: Dict[str, str]) -> Dict[str, Any]:
    ...
    for task_pattern, route_json in routes_data.items():
        try:
            ...
            logger.warning(
                f"[CELERY_CONFIG_LOADER] Erreur désérialisation route {task_pattern}: {e}"
            )
            continue
...
def deserialize_base_config_from_redis(
    base_config_data: Dict[str, str],
) -> Dict[str, Any]:
    """
    ...
    for key, value_json in base_config_data.items():
        try:
            ...
            logger.warning(
                f"[CELERY_CONFIG_LOADER] Erreur désérialisation config {key}: {e}"
            )
            continue
...
def load_celery_config_from_redis(
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"[CELERY_CONFIG_LOADER] Tentative de chargement (attempt {attempt + 1}/{max_retries})"
            )
            ...
            if not version:
                logger.warning(
                    "[CELERY_CONFIG_LOADER] Aucune configuration trouvée dans Redis (clé: celery_config:version)"
                )
                if attempt == max_retries - 1:
                    # Dernière tentative - utiliser le fallback
                    logger.warning(
                        "[CELERY_CONFIG_LOADER] >>> UTILISATION DU FALLBACK (attempt {})".format(
                            attempt + 1
                        )
                    )
                    ...
                    logger.info(
                        f"[CELERY_CONFIG_LOADER] Attente {retry_delay}s avant nouvelle tentative..."
                    )
            ...
            if not base_config_data:
                logger.warning(
                    "[CELERY_CONFIG_LOADER] Aucune config de base trouvée dans Redis"
                )
                if attempt == max_retries - 1:
                    return get_fallback_config()
                else:
            ...
            logger.info(
                "[CELERY_CONFIG_LOADER] Configuration chargée avec succès depuis Redis"
            )
            logger.info(
                f"[CELERY_CONFIG_LOADER] {len(queues)} queues, {len(routes)} routes, {len(base_config)} paramètres de base"
            )
            ...
            last_error = e
            logger.error(
                f"[CELERY_CONFIG_LOADER] Erreur lors du chargement (attempt {attempt + 1}): {str(e)}"
            )

            if attempt == max_retries - 1:
                logger.error(
                    f"[CELERY_CONFIG_LOADER] Toutes les tentatives ont échoué: {str(last_error)}"
                )
                logger.info(
                    "[CELERY_CONFIG_LOADER] Utilisation de la configuration de fallback"
                )
                return get_fallback_config()
            else:
                logger.info(
    ...
    logger.error(
        "[CELERY_CONFIG_LOADER] Échec complet du chargement - utilisation du fallback"
    )
    return get_fallback_config()
...
def wait_for_celery_config(timeout: int = 30) -> bool:
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if version:
                logger.info(
                    f"[CELERY_CONFIG_LOADER] Configuration Celery disponible (version: {version})"
                )
                return True

        except Exception as e:
            logger.warning(
                f"[CELERY_CONFIG_LOADER] Erreur lors de la vérification: {str(e)}"
            )

        time.sleep(1)

    logger.warning(
        f"[CELERY_CONFIG_LOADER] Timeout: configuration Celery non disponible après {timeout}s"
    )
    return False
