...
def _get_encryption_key() -> bytes:
    ...
    try:
        ...
        raise RuntimeError(
            f"ENCRYPTION_KEY is invalid: {str(e)}. Please provide a valid Fernet key."
        )
...
def decrypt_value(encrypted_value: str) -> str:
    ...
    if not encrypted_value or not isinstance(encrypted_value, str):
        logger.warning(
            f"Valeur vide ou invalide pour le déchiffrement: {encrypted_value}"
        )
        return ""

    logger.info(
        f"Tentative de déchiffrement de la valeur: {encrypted_value[:50]}..."
    )  # Log des 50 premiers caractères
