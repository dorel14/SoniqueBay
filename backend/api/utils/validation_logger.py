...
def log_validation_error(
    ...
    try:
        ...
        logger.error(
            f"ERREUR 422 VALIDATION: {json.dumps(log_data, indent=2, default=str)}"
        )
...
def log_schema_validation_debug(
    schema_name: str, data: Any, validation_result: bool = True
):
...
def create_validation_summary(
    ...
    for error in validation_errors:
        for err in error.errors():
            ...
            summary["field_errors"][field].append(
                {"type": error_type, "message": err["msg"]}
            )
    ...
    if summary["error_types"]:
        ...
        summary["common_patterns"].append(
            f"Type d'erreur le plus fréquent: {most_common_type} ({summary['error_types'][most_common_type]} fois)"
        )
