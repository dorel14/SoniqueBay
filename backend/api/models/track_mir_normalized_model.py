...
class TrackMIRNormalized(Base, TimestampMixin):
    ...
    bpm: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Tempo normalisé en battements par minute"
    )
    key: Mapped[str] = mapped_column(
        String(10), nullable=True, doc="Tonalité normalisée (C, C#, D, etc.)"
    )
    scale: Mapped[str] = mapped_column(
        String(10), nullable=True, doc="Mode (major/minor)"
    )
    ...
    loudness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: very_quiet [0] -> very_loud [1]"
    )
    tempo: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: slow [0] -> fast [1]"
    )
    energy: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: low [0] -> high [1]"
    )
    valence: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: negative [0] -> positive [1]"
    )
    acousticness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: electronic [0] -> acoustic [1]"
    )
    instrumentalness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: vocal [0] -> instrumental [1]"
    )
    speechiness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: music [0] -> speech [1]"
    )
    liveness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: studio [0] -> live [1]"
    )
    dynamic_range: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: compressed [0] -> dynamic [1]"
    )
    spectral_complexity: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: simple [0] -> complex [1]"
    )
    harmonic_complexity: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: simple [0] -> complex [1]"
    )
    perceptual_roughness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: smooth [0] -> rough [1]"
    )
    auditory_roughness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Normalisé: pleasant [0] -> harsh [1]"
    )

    # Genres normalisés
    genre_main: Mapped[str] = mapped_column(
        String(100), nullable=True, doc="Genre principal normalisé"
    )
    genre_secondary: Mapped[list[str]] = mapped_column(
        JSON, nullable=True, doc="Genres secondaires (tableau JSON)"
    )

    # Clé Camelot pour les mixes DJ
    camelot_key: Mapped[str] = mapped_column(
        String(5), nullable=True, doc="Clé Camelot pour DJ (ex: 8B, 5A)"
    )

    # Score de confiance global (nouveau schéma)
    confidence_score: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score de confiance global [0.0-1.0]"
    )

    # Date de normalisation (nouveau schéma)
    normalized_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="Date de normalisation"
    )
    ...
    normalization_version: Mapped[str] = mapped_column(
        String(20), nullable=True, doc="Version de l'algorithme de normalisation"
    )
    ...
    track: Mapped["Track"] = relationship(
        "Track", back_populates="mir_normalized", lazy="selectin"
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "normalization_date": (
                self.normalization_date.isoformat() if self.normalization_date else None
            ),
            # Métadonnées
