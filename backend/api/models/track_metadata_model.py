...
class TrackMetadata(Base, TimestampMixin):
    ...
    metadata_value: Mapped[str] = mapped_column(
        Text, nullable=True, doc="Valeur de la métadonnée (stockée comme texte)"
    )
    ...
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="Date de création de la métadonnée"
    )

    # Relations
    track: Mapped["Track"] = relationship(
        "Track", back_populates="metadata_entries", lazy="selectin"
    )
