...
class TrackMIRScores(Base, TimestampMixin):
    ...
    energy_score: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score d'énergie [0.0-1.0]"
    )
    mood_valence: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score de valence émotionnelle [-1.0 à +1.0]"
    )
    dance_score: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score de danseabilité [0.0-1.0]"
    )
    acousticness: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score d'acousticité [0.0-1.0]"
    )
    complexity_score: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score de complexité [0.0-1.0]"
    )
    emotional_intensity: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Intensité émotionnelle [0.0-1.0]"
    )

    # Date du calcul
    calculated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="Date du calcul des scores"
    )

    # Relations
    track: Mapped["Track"] = relationship(
        "Track", back_populates="mir_scores", lazy="selectin"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_track_mir_scores_multi", "energy_score", "dance_score", "acousticness"
        ),
    )
