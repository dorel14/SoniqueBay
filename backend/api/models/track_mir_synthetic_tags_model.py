...
class TrackMIRSyntheticTags(Base):
    ...
    tag_name: Mapped[str] = mapped_column(
        String(100), nullable=False, doc="Nom du tag synthétique"
    )
    tag_score: Mapped[float] = mapped_column(
        Float, nullable=True, doc="Score du tag [0.0-1.0]"
    )
    ...
    tag_source: Mapped[str] = mapped_column(
        String(50), nullable=True, doc="Source du tag: calculated, llm, manual"
    )

    # Date de création du tag
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=True, doc="Date de création du tag"
    )
    ...
    track: Mapped["Track"] = relationship(
        "Track", back_populates="mir_synthetic_tags", lazy="selectin"
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "date_added": (
                ...
                if getattr(self, "date_added", None)
                else None
            ),
            "date_modified": (
                self.date_modified.isoformat()
                if getattr(self, "date_modified", None)
                else None
            ),
        }
