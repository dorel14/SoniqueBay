...
class Conversation(Base, TimestampMixin):
    ...
    system_context: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    ...
    session: Mapped[Optional["ChatSession"]] = relationship(
        "ChatSession", back_populates="conversations"
    )
    ...
    def to_dict(self, include_summary: bool = True) -> dict:
        ...
        if include_summary and self.summary:
            ...
            result["summary_generated_at"] = (
                self.summary_generated_at.isoformat()
...
class ChatMessage(Base, TimestampMixin):
    ...
    replies: Mapped[List["ChatMessage"]] = relationship(
        "ChatMessage", back_populates="parent"
    )
    ...
    def to_dict(
        self, include_content: bool = True, include_embedding: bool = False
    ) -> dict:
        """Convertit en dictionnaire."""
        result = {
            "id": str(self.id),
            ...
            "message_timestamp": (
                self.message_timestamp.isoformat() if self.message_timestamp else None
            ),
            "created_at": self.date_added.isoformat() if self.date_added else None,
...
Index("idx_chat_messages_conv_role", ChatMessage.conversation_id, ChatMessage.role)
...
Index(
    "idx_conversations_type_user", Conversation.conversation_type, Conversation.user_id
)
