...
class TrackVectorizationService:
    ...
    def create_track_embedding(
        ...
        try:
            ...
            if year and isinstance(year, (int, str)):
                try:
                    year_int = (
                        int(str(year)[:4]) if isinstance(year, str) else int(year)
                    )
            ...
            target_length = (
                len(self.audio_features) + 5 + 12 + 3
            )  # audio + genre + key + bpm + duration + year
            ...
            if np.std(features_array) > 0:
                features_array = (features_array - np.mean(features_array)) / np.std(
                    features_array
                )
            ...
            logger.debug(
                f"Created embedding for track with {len(embedding)} dimensions"
            )
            return embedding
    ...
    def _encode_key(self, key: str) -> List[float]:
        ...
        for known_key, vector in self.key_mapping.items():
            if clean_key.replace("#", "").replace("b", "") == known_key.replace(
                "#", ""
            ).replace("b", ""):
                return vector
    ...
    def batch_create_embeddings(
        ...
        for track_data in tracks_data:
            ...
            try:
                ...
                if embedding:
                    successful.append({"track_id": track_id, "embedding": embedding})
                else:
                    failed.append(
                        {"track_id": track_id, "error": "Failed to create embedding"}
                    )
            except Exception as e:
                failed.append({"track_id": track_id, "error": str(e)})
