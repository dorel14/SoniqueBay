...
def _get_llm_lock() -> asyncio.Lock:
    ...
    if _llm_service_lock is None:
        ...
        try:
            ...
            logger.warning(
                f"[LLM] Pas d'event loop actif à l'initialisation du lock: {e}"
            )
...
class LLMService:
    ...
    def get_model(
        ...
        if self.provider_type == "koboldcpp":
            ...
            logger.debug(
                f"[LLM] Utilisation de KoboldCPP avec modèle={model_name} url={self.base_url}/v1"
            )
        else:
            # Ollama par défaut
            provider = OllamaProvider(base_url=self.base_url)
            logger.debug(
                f"[LLM] Utilisation d'Ollama avec modèle={model_name} url={self.base_url}"
            )
    ...
    async def _stream_chat_response(
        ...
        logger.debug(
            f"[LLM] Démarrage du streaming avec {self.provider_type} sur {self.base_url}"
        )
                    ...
                    async for line in response.aiter_lines():
                        ...
                        if line.startswith("data: "):
                            ...
                            if data_str.strip() == "[DONE]":
                                logger.debug(
                                    "[LLM] Fin du stream SSE détectée ([DONE])"
                                )
                                break
                    ...
                    async for line in response.aiter_lines():
                        ...
                        try:
                            ...
                            if data.get("done", False):
                                logger.debug(
                                    "[LLM] Fin du stream Ollama détectée (done=true)"
                                )
                                break
                        except json.JSONDecodeError:
                            continue
