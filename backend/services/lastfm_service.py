...
class LastFMService:
    ...
    async def _fetch_album_info(self, artist_name: str, album_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            lastfm_artist = self.network.get_artist(artist_name)
            lastfm_album = lastfm_artist.get_album(album_name)  # type: ignore
    ...
    async def _fetch_track_info(self, artist_name: str, track_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            lastfm_artist = self.network.get_artist(artist_name)
            lastfm_track = lastfm_artist.get_track(track_name)  # type: ignore
    ...
    async def _fetch_artist_image(self, artist_name: str) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            for size in preferred_sizes:
                try:
                    image_url = lastfm_artist.get_image(size)  # type: ignore  # type: ignore  # type: ignore  # type: ignore
                    if image_url:
    ...
    def _get_artist_images(self, artist: pylast.Artist) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for size in sizes:
                try:
                    image_url = artist.get_image(size)  # type: ignore  # type: ignore
                    if image_url:
                        images.append({
                            "size": size,
                            "url": image_url
    ...
    def _get_album_images(self, album: pylast.Album) -> List[Dict[str, Any]]:
        ...
        try:
            images = album.get_images()  # type: ignore  # type: ignore
            return [{
                "size": img.get("size"),
                "url": img.get("#text")
    ...
    def _get_track_images(self, track: pylast.Track) -> List[Dict[str, Any]]:
        ...
        try:
            images = track.get_images()  # type: ignore
            return [{
                "size": img.get("size"),
                "url": img.get("#text")
            } for img in images if img.get("size") and img.get("#text")]
