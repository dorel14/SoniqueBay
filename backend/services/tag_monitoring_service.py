...
class TagChangeDetector:
    ...
    async def get_current_tags(self) -> Dict[str, Union[Set[str], int]]:
        """
    ...
    def calculate_tags_signature(self, tags: Dict[str, Union[Set[str], int]]) -> str:
        """
        ...
        tag_data = {
            'genres': sorted(list(tags['genres']) if isinstance(tags['genres'], set) else []),
            'mood_tags': sorted(list(tags['mood_tags']) if isinstance(tags['mood_tags'], set) else []),
            'genre_tags': sorted(list(tags['genre_tags']) if isinstance(tags['genre_tags'], set) else []),
        ...
        try:
            ...
            if self.last_check is None:
                ...
                return {
                    ...
                    'details': {
                        'new_genres': len(current_tags['genres']) if isinstance(current_tags['genres'], (set, list)) else 0,
                        'new_moods': len(current_tags['mood_tags']) if isinstance(current_tags['mood_tags'], (set, list)) else 0,
                        'new_genre_tags': len(current_tags['genre_tags']) if isinstance(current_tags['genre_tags'], (set, list)) else 0,
...
class TaskIQPublisher:
    """Publieur pour déclencher les tâches TaskIQ de retrain."""
    
    def __init__(self):
        """Initialise le publisher TaskIQ."""
        pass
    
    async def trigger_retrain_task(self, trigger_info: Dict[str, Any]) -> bool:
        """
        Déclenche une tâche TaskIQ de retrain et publie les notifications SSE.
        ...
        try:
            ...
            # TODO: Déclencher la tâche TaskIQ de retrain
            # from backend_worker.workers.vectorization.monitoring_worker import trigger_vectorizer_retrain
            # task_result = await trigger_vectorizer_retrain.kiq(retrain_request=task_args)

            logger.warning(f"[TASKIQ] Retrain non déclenché (import manquant): {trigger_info['reason']}")

            # TODO: Publier les notifications SSE pour l'UI
            # await self._publish_sse_notifications(trigger_info, "mock-task-id")
...
RedisPublisher = TaskIQPublisher
CeleryTaskPublisher = TaskIQPublisher  # Alias rétrocompatible
...
class TagMonitoringService:
    ...
    def __init__(self):
        ...
        self.publisher = TaskIQPublisher()
        self.check_interval_minutes: Union[int, float] = 60  # Vérification toutes les heures
    ...
    async def check_and_publish_if_needed(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if retrain_decision['should_retrain']:
                ...
                # Déclencher la tâche TaskIQ de retrain
                taskiq_success = await self.publisher.trigger_retrain_task(retrain_decision)
                ...
                return {
                    ...
                    'taskiq_triggered': taskiq_success,
                    'timestamp': datetime.now().isoformat(),
