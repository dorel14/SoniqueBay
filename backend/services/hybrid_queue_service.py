...
from backend.services.local_fallback_queue_service import local_fallback_queue_service
...
class HybridQueueService:
            ...
            for attempt in range(self.retry_config['max_retries']):
                try:
                    ...
                    if success:
                        ...
                        if self.redis_service.redis:
                            try:
                                # Note: Cannot await in sync function, skipping Redis info
                                logger.error(f"[HYBRID_QUEUE DIAGNOSTIC] Redis info skipped in sync context")
                            except Exception as info_error:
                                logger.error(f"[HYBRID_QUEUE DIAGNOSTIC] Erreur diagnostic Redis: {info_error}")
    ...
    def complete_task(self, queue_name: str, task_id: str, success: bool = True,
        ...
        try:
            local_success = self.local_service.complete_task(queue_name, task_id, success, error_message or "")
        except Exception as e:
    ...
    def get_system_health(self) -> Dict[str, Any]:
        ...
        if self.redis_available:
            try:
                if self._test_redis_connection():
                    # Note: Cannot await Redis info in sync function
                    health["redis_memory"] = {"used": 0, "peak": 0, "max": 0}  # type: ignore
                    health["redis_connected_clients"] = 0  # type: ignore
                else:
