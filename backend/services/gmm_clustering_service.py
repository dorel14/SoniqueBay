...
class GMMClusteringService:
    ...
    def _fallback_kmeans(
        ...
        kmeans_params = {
            ...
            'random_state': self.random_state,
            'init': 'k-means++',  # Correct parameter for KMeans
            'n_init': 10,
            'max_iter': 300,
        }
    ...
    def score(
        ...
        if self._model_type == 'gmm' and self._gmm_model is not None:
            if metric == 'bic':
                return self._gmm_model.bic(embeddings)  # type: ignore
            elif metric == 'aic':
                return self._gmm_model.aic(embeddings)  # type: ignore
            elif metric == 'log_likelihood':
                return self._gmm_model.score(embeddings)  # type: ignore
            else:
