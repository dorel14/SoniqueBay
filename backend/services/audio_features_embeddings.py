...
class AudioFeaturesEmbeddingService:
    ...
    def _init_mir_service(self) -> None:
        ...
        try:
            from backend.services.mir_scoring_service import MIRScoringService
    ...
    def aggregate_track_features_to_artist(
        ...
        if weights is None:
            weights_array: np.ndarray = np.ones(n_tracks, dtype=np.float32)  # type: ignore
        else:
            weights_array = np.array(weights, dtype=np.float32)  # type: ignore

        # Normaliser les poids
        weights_array = weights_array / weights_array.sum()  # type: ignore
        ...
        for i, track in enumerate(track_features):
            try:
                ...
                centroid += track_vector * weights_array[i]  # type: ignore
            except Exception as e:
                logger.warning(f"[AudioFeaturesEmbedding] Erreur vecteur track {i}: {e}")
                # Skip cette track avec poids redistribué
                weights_array[i] = 0.0  # type: ignore

        # Normaliser les poids après过滤无效tracks
        if weights_array.sum() > 0:  # type: ignore
            weights_array = weights_array / weights_array.sum()  # type: ignore
            centroid = np.zeros(64, dtype=np.float32)
            for i, track in enumerate(track_features):
                if weights_array[i] > 0:  # type: ignore
                    track_vector = self.audio_features_to_vector(track)
                    centroid += track_vector * weights_array[i]  # type: ignore
