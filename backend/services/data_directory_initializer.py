            ...
            try:
                ...
                if hasattr(os, 'getuid') and hasattr(os, 'getgid'):
                    ...
                    if grp is not None and hasattr(grp, 'getgrgid'):
                        group = grp.getgrgid(gid)  # type: ignore
                    else:
                ...
                if HAS_PWD_GRP:
                    # Check if the functions exist before calling them
                     ...
                     if getpwuid_fn and callable(getpwuid_fn):
                         ...
                         result["owner"] = getattr(user_info, 'pw_name', "unknown")  # type: ignore
                     else:
                     ...
                     if getgrgid_fn and callable(getgrgid_fn):
                         ...
                         result["group"] = getattr(group_info, 'gr_name', "unknown")  # type: ignore
                     else:
        ...
        try:
            ...
            try:
                if HAS_PWD_GRP and self.current_user["uid"] != -1:
                    ...
                    if hasattr(os, 'chown'):
                        os.chown(path, self.current_user["uid"], self.current_user["gid"])  # type: ignore
                        result["ownership_set"] = True
                    else:
                        logger.warning("os.chown not available on this platform")
