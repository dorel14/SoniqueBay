...
from backend.services.entity_manager import (
    create_or_get_artists_batch,
    create_or_get_albums_batch,
    create_or_update_tracks_batch,
    create_or_update_cover
)
from backend.tasks.audio_analysis_tasks import analyze_track_audio_features
from backend.tasks.metadata import enrich_artist_task, enrich_album_task
...
class ScanOptimizer:
        ...
        async def process_single_file(file_path_bytes: bytes) -> Optional[Dict]:
            async with self.file_semaphore:
                try:
                    ...
                    from backend.services.music_scan import process_file
    ...
    async def analyze_audio_batch(self, track_data_list: List[Dict]) -> List[Dict]:
        """
        Lance l'analyse audio de plusieurs tracks via des tâches TaskIQ en arrière-plan.

        Les tâches TaskIQ sont envoyées à la queue 'audio_analysis' et traitées
        de manière asynchrone. Les résultats sont stockés directement en base de données
        par les workers TaskIQ.
        ...
        # Envoyer les tâches TaskIQ pour l'analyse audio
        logger.info("🚀 Envoi des tâches TaskIQ vers la queue 'audio_analysis'...")
        for track_data in track_data_list:
            ...
            if track_id and file_path:
                try:
                    # Envoyer la tâche TaskIQ à la queue 'audio_analysis'
                    logger.debug(f"  Envoi tâche pour track {track_id}: {file_path}")
                    await analyze_track_audio_features.kiq(track_id, file_path, tags)
                    tasks_sent += 1
                except Exception as e:
        ...
        processing_time = time.time() - start_time
        logger.info(f"✅ Analyse audio batch: {tasks_sent}/{len(track_data_list)} tâches TaskIQ envoyées en {processing_time:.2f}s")
        ...
        try:
            ...
            if unique_artists:
                ...
                for artist in artist_map.values():
                    await enrich_artist_task.kiq(artist['id'])
            ...
            if unique_albums:
                ...
                for album in album_map.values():
                    await enrich_album_task.kiq(album['id'])
            ...
            if all_cover_tasks:
                ...
                for cover_task in all_cover_tasks:
                    if cover_task[0] == "album" and len(cover_task) >= 5:
                        ...
                        artist = artist_map.get(artist_name)
                        if artist:
                            from backend.services.entity_manager import process_artist_covers
