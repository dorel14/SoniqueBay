...
from backend.services.coverart_service import get_cover_schema, get_cover_types
...
async def on_artists_inserted_callback(artist_ids: List[int]) -> None:
    ...
    try:
        ...
        task_result = await process_artist_images.kiq(artist_ids)
         
        logger.info(f"[CALLBACK] Tâche d'images artistes déclenchée avec ID: {task_result}")
...
async def on_albums_inserted_callback(album_ids: List[int]) -> None:
    ...
    try:
        ...
        task_result = await process_album_covers.kiq(album_ids)
         
        logger.info(f"[CALLBACK] Tâche de covers albums déclenchée avec ID: {task_result}")
        ...
        for track_data in track_data_list:
            try:
                ...
                if track_data.get("artist_path") and track_data.get("track_artist_id"):
                    ...
                    from backend.services.image_service import get_artist_images
