...
async def cleanup_old_data_task(days_old: int = 30) -> dict:
    ...
    try:
        # Nettoyage via l API de maintenance
        # Délégation au service de maintenance
        logger.info(f"[TASKIQ|MAINTENANCE] Nettoyage des données de plus de {days_old} jours (effectué via API)")
        result = {
            "cleaned": True,
            "days_old": days_old,
            "items_cleaned": 0,
...
async def archive_old_logs_task(days_to_keep: int = 30) -> dict:
    ...
    try:
        ...
        # Archivage via l API de maintenance
        result = {
