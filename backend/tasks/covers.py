...
# from backend.workers.services.cover_orchestrator_service import cover_orchestrator_service
# from backend.workers.services.cover_types import CoverProcessingContext, ImageType, TaskType
...
async def extract_embedded_task(file_paths: List[str]) -> Dict[str, Any]:
    ...
    for file_path in file_paths:
        try:
            ...
            from backend.services.music_scan import get_cover_art
...
async def process_track_covers_batch(album_covers: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        for cover_data in album_covers:
            # Utilisation de l API pour le traitement des images
            # Traitement via l API REST
            context_data = {
        ...
        result = {
            ...
            "success": True,
            "message": f"Traitement de {len(album_covers)} covers effectué (effectué via API)"
        }
...
async def process_artist_images(artist_ids: List[int], priority: str = "normal") -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "success": True,
            "message": f"Traitement de {len(artist_ids)} images d'artistes effectué (effectué via API)"
        }
...
async def process_album_covers(album_ids: List[int], priority: str = "normal") -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "success": True,
            "message": f"Traitement de {len(album_ids)} covers d'albums effectué (effectué via API)"
        }
...
async def process_artist_images_batch(artist_images: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "success": True,
            "message": f"Traitement de {len(artist_images)} lots d'images d'artistes effectué (effectué via API)"
        }
...
async def extract_artist_images(file_paths: List[str]) -> Dict[str, Any]:
    ...
    for artist_path, files_in_folder in artist_folders.items():
        try:
            ...
            from backend.services.music_scan import extract_artist_images as async_extract_artist_images
