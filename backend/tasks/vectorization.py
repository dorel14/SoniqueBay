...
def _get_model():
    """Retourne le modèle SentenceTransformer en cache (chargé une seule fois)."""
    global _model_cache
    if _model_cache is None:
        from sentence_transformers import SentenceTransformer
        logger.info(f"[TASKIQ|VECTOR] Chargement du modèle {EMBEDDING_MODEL_NAME}...")
        _model_cache = SentenceTransformer(EMBEDDING_MODEL_NAME)
        logger.info(f"[TASKIQ|VECTOR] Modèle {EMBEDDING_MODEL_NAME} chargé")
    return _model_cache
...
async def calculate_vector_task(track_id: int, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        # Modele en cache via _get_model()
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            response = await client.post(
                ...
                json={"vector": embedding_list, "model": EMBEDDING_MODEL_NAME}
            )
        ...
        return {
            ...
            'calculation_time': calculation_time,
            'embedding_model': EMBEDDING_MODEL_NAME if 'EMBEDDING_MODEL_NAME' in locals() else 'unknown'
        }
