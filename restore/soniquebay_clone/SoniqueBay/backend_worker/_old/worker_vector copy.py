...
from backend_worker.services.vectorization_service import (
    vectorize_single_track, vectorize_and_store_batch,
...
@celery.task(name="worker_vector.train_vectorizer", queue="worker_vector")
def train_vectorizer_task() -> Dict[str, Any]:
    """
    Tâche d'entraînement du vectorizer avec tous les tags de la base de données.

    Returns:
        Résultat de l'entraînement
    """
    try:
        logger.info("[WORKER_VECTOR] Démarrage entraînement vectorizer avec tags BDD")

        # Récupérer tous les genres et moods tags de la BDD
        all_genre_tags, all_mood_tags = asyncio.run(_get_all_tags_from_db())

        # Créer le service de vectorisation
        service = VectorizationService()

        # Entraîner le vectorizer avec les tags
        result = asyncio.run(service.train_with_tags(all_genre_tags, all_mood_tags))

        logger.info(f"[WORKER_VECTOR] Entraînement vectorizer terminé: {result}")
        return result

    except Exception as e:
        logger.error(f"[WORKER_VECTOR] Erreur entraînement vectorizer: {str(e)}", exc_info=True)
        return {"error": str(e)}
...
async def _get_all_tags_from_db() -> tuple[List[str], List[str]]:
    """
    Récupère tous les genres et moods tags de la base de données.

    Returns:
        Tuple (genre_tags, mood_tags)
    """
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            # Récupérer tous les genres
            genre_response = await client.get("http://api:8001/api/genres/")
            genre_tags = []
            if genre_response.status_code == 200:
                genres = genre_response.json()
                genre_tags = [genre["name"] for genre in genres]

            # Récupérer tous les tags d'humeur
            mood_response = await client.get("http://api:8001/api/tags/moods")
            mood_tags = []
            if mood_response.status_code == 200:
                moods = mood_response.json()
                mood_tags = [mood["name"] for mood in moods]

            logger.info(f"[WORKER_VECTOR] Récupéré {len(genre_tags)} genres et {len(mood_tags)} moods tags")
            return genre_tags, mood_tags

    except Exception as e:
        logger.error(f"[WORKER_VECTOR] Erreur récupération tags BDD: {str(e)}")
        return [], []